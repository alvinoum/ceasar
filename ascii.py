"""
Declaring my class for colours, below are a  few different ones I'll use throughout my program
...
"""
here we are deciding weather we want to encrypt or decrypt a message, the getMode() will allow the user to type in what they want so either encrypt,e or decrypt, d. where we are starting a while loop, it will ask you to type in, as you can see we have rawinput  and allows you to add a value to mode integer, you selected %s so it's looking for a text based answer, when selected it will print it back to you.
 
"""
def getMode(): #definition called getmode, break program into sections
    while True: # while loop start
        print '*' * 50 # used for a border prints * fifty times
        ...
        mode = raw_input(">")#allows user to input a value on screen
        print text_colors.BLUE + "\nyou selected %s" % mode + text_colors.ENDC #reply what you typed
        print '*' * 50
        #if mode in "encrypt e decrypt d".split():
        return mode # return the value of mode
...
def getKey(): # function for the key
    ...
    while True:#keeps looping intill a value has been entered
        ...
        if (key >= 1 and key <= 26): #if what's entered is greater than 1 and less then 26
...
"""
This function does the encryption and decryption in three parts mode sets the function to encryption mode or decryption mode, message is the plain text (or cipher text) to be encrypted (or decrypted), key is the key that is used in this cipher.in crypt the letters are called symbols, starts a for loop for message. is alpha is a string looking for alphabet, is upper looks for upper case letters and islower looks for lower, translated changes the encryption and decrypted charters in this string, if the symbol isn't a letter then it will either ignore it and leave it same or shut down.
"""
...
These changes ask the user for the key if they aren't in brute force mode if they are it won't ask
if they are it goes from 1 to 26 showing all results
...
## code is from http://www.hackinginsider.com/2015/10/a-guide-to-the-caesar-cipher-and-coding-one-in-python/